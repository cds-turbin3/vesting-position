...
use vesting_litesvm_tests::merkle::{default_merkle, random_proofs, MerkleTree};
...
fn mint_alice_position(merkle: &MerkleTree, world: &mut TestCampaign) -> (WhitelistUser, Pubkey) {
...
fn partial_claim(world: &mut TestCampaign, user: &WhitelistUser, asset: Pubkey) {
...
fn close_campaign_succeeds_when_vault_empty() {
    ...
    let merkle = default_merkle();
    let alice = load_whitelist_user(&merkle, WHITELISTED_1);
    let mut world = TestCampaign::initialized(
        &merkle,
        CampaignConfig {
