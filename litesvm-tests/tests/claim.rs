//! Claim integration tests, on the source-free harness.
//!
//! Definitions:
...
//!
//! The old suite spelled every subsequent claim out inline (build the bundle,
//! `Claim { None, None, name, uri }`, send, expire the blockhash); that block
//! repeated a dozen times. Here the world owns the verbs
//! (`first_claim_ok`/`subsequent_claim_ok`/`subsequent_claim_err`), so each
//! scenario reads as the sequence of claims it actually is.
...
use vesting_litesvm_tests::merkle::{default_merkle, leaf_hash, random_proofs, verify, MerkleTree};

/// Default merkle fixture + an initialized campaign.
fn setup() -> (MerkleTree, TestCampaign) {
    let merkle = default_merkle();
    let world = TestCampaign::initialized(&merkle, CampaignConfig::default());
    (merkle, world)
}
