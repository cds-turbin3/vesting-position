...
//! `vesting_positions::utils::merkletree`) and the test-side tree loader. Pure
//! crypto over `Pubkey` and bytes (sha3 + serde_json), so nothing here reaches
//! for the program crate.
